...
{
    I2Cdev::readWord(dev->addr, 0x88, &dev->dig_T1);
    I2Cdev::readWord(dev->addr, 0x8a, (uint16_t*)&dev->dig_T2);
    I2Cdev::readWord(dev->addr, 0x8c, (uint16_t*)&dev->dig_T3);
    I2Cdev::readWord(dev->addr, 0x8e, &dev->dig_P1);
    I2Cdev::readWord(dev->addr, 0x90, (uint16_t*)&dev->dig_P2);
    I2Cdev::readWord(dev->addr, 0x92, (uint16_t*)&dev->dig_P3);
    I2Cdev::readWord(dev->addr, 0x94, (uint16_t*)&dev->dig_P4);
    I2Cdev::readWord(dev->addr, 0x96, (uint16_t*)&dev->dig_P5);
    I2Cdev::readWord(dev->addr, 0x98, (uint16_t*)&dev->dig_P6);
    I2Cdev::readWord(dev->addr, 0x9a, (uint16_t*)&dev->dig_P7);
    I2Cdev::readWord(dev->addr, 0x9c, (uint16_t*)&dev->dig_P8);
    I2Cdev::readWord(dev->addr, 0x9e, (uint16_t*)&dev->dig_P9);
...
static esp_err_t read_hum_calibration_data(bmp280_t *dev)
{
    ...
    I2Cdev::readByte(dev->addr, 0xa1, &dev->dig_H1);
    I2Cdev::readWord(dev->addr, 0xe1, (uint16_t*)&dev->dig_H2);
    I2Cdev::readByte(dev->addr, 0xe3, &dev->dig_H3);
    I2Cdev::readWord(dev->addr, 0xe4, &h4);
    I2Cdev::readWord(dev->addr, 0xe5, &h5);
    I2Cdev::readByte(dev->addr, 0xe7, (uint8_t*)&dev->dig_H6);
...
esp_err_t bmp280_init(bmp280_t *dev, bmp280_params_t *params)
{
    ...
    CHECK_LOGE(dev, I2Cdev::readByte(dev->addr, BMP280_REG_ID, &dev->id), "Sensor not found");
    ...
    // Soft reset.
    CHECK_LOGE(dev, I2Cdev::writeByte(dev->addr, BMP280_REG_RESET, BMP280_RESET_VALUE), "Failed resetting sensor");

    // Wait until finished copying over the NVP data.
    while (1)
    {
        uint8_t status;
        if (!I2Cdev::readByte(dev->addr, BMP280_REG_STATUS, &status) && (status & 1) == 0)
            break;
    }
    ...
    CHECK_LOGE(dev, I2Cdev::writeByte(dev->addr, BMP280_REG_CONFIG, config), "Failed configuring sensor");
    ...
    if (dev->id == BME280_CHIP_ID)
    {
        ...
        CHECK_LOGE(dev, I2Cdev::writeByte(dev->addr, BMP280_REG_CTRL_HUM, ctrl_hum), "Failed controlling sensor");
    }

    ESP_LOGD(TAG, "Writing ctrl reg=%x", ctrl);
    CHECK_LOGE(dev, I2Cdev::writeByte(dev->addr, BMP280_REG_CTRL, ctrl), "Failed controlling sensor");
...
esp_err_t bmp280_force_measurement(bmp280_t *dev)
{
    ...
    uint8_t ctrl;
    I2Cdev::readByte(dev->addr, BMP280_REG_CTRL, &ctrl);
    ctrl &= ~0b11;  // clear two lower bits
    ctrl |= BMP280_MODE_FORCED;
    ESP_LOGD(TAG, "Writing ctrl reg=%x", ctrl);
    CHECK_LOGE(dev, I2Cdev::writeByte(dev->addr, BMP280_REG_CTRL, ctrl), "Failed starting forced mode");
...
esp_err_t bmp280_is_measuring(bmp280_t *dev, bool *busy)
{
    ...
    uint8_t status[2];
    I2Cdev::readByte(dev->addr, BMP280_REG_CTRL, &status[1]);
    I2Cdev::readByte(dev->addr, BMP280_REG_STATUS, &status[0]);
...
esp_err_t bmp280_read_fixed(bmp280_t *dev, int32_t *temperature, uint32_t *pressure, uint32_t *humidity)
{
    ...
    CHECK_LOGE(dev, I2Cdev::readBytes(dev->addr, 0xf7, size, data), "Failed reading");
