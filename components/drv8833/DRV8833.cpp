...
DRV8833::DRV8833(int pin1, int pin2, int pwm1, int pwm2, int minInput, int maxInput, int neutralWidth, boolean invert, boolean doublePWM) { // Constructor
    _pin1 = pin1;
    _pin2 = pin2;
    _pwm1 = pwm1;
    _pwm2 = pwm2;
    ...
    ledcSetup(pwm1, 500, 8); // channel, frequency, resolution
    ledcSetup(pwm2, 500, 8);
    ledcAttachPin( pin1, pwm1 );
    ledcAttachPin( pin2, pwm2 );
}
    ...
    if (_doublePWM) { // Mode with two PWM capable pins (both pins must be PWM capable!) -----
        if (!_brake) { // Coast mode (fast decay)
            if (_controlValueRamp >= _maxNeutral) { // Forward
                digitalWrite(_pin1, LOW);
                ledcWrite(_pwm2, map(_controlValueRamp, _maxNeutral, _maxInput, 0, _maxPWM));
            }
            else if (_controlValueRamp <= _minNeutral) { // Reverse
                digitalWrite(_pin2, LOW);
                ledcWrite(_pwm1, map(_controlValueRamp, _minNeutral, _minInput, 0, _maxPWM));
            }
            else { // Neutral
                digitalWrite(_pin1, LOW);
                digitalWrite(_pin2, LOW);
            ...
            if (_controlValueRamp >= _maxNeutral) { // Forward
                ...
                ledcWrite(_pwm1, 255 - map(_controlValueRamp, _maxNeutral, _maxInput, 0, _maxPWM));
            }
            else if (_controlValueRamp <= _minNeutral) { // Reverse
                digitalWrite(_pin1, HIGH);
                ledcWrite(_pwm2, 255 - map(_controlValueRamp, _minNeutral, _minInput, 0, _maxPWM));
            }
            else { // Neutral
                if (_neutralBrake) {
