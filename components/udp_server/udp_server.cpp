...
uint8_t WiFiUDP::begin(in_addr_t address, uint16_t port){
  stop();
...
uint8_t WiFiUDP::begin(uint16_t p){
  return begin(INADDR_ANY, p);
}

uint8_t WiFiUDP::beginMulticast(in_addr_t a, uint16_t p){
  if(begin(INADDR_ANY, p)){
    if(a != 0){
      ESP_LOGI(TAG, "starting multicast on address %s", inet_ntoa(a));
      struct ip_mreq mreq;
      mreq.imr_multiaddr.s_addr = a;
      mreq.imr_interface.s_addr = INADDR_ANY;
      if (setsockopt(udp_server, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
          ESP_LOGE(TAG,"could not join igmp: %d", errno);
          stop();
          return 0;
      }
      multicast_ip = a;
    } else {
      ESP_LOGI(TAG, "multicast not set");
    }
    return 1;
  }
  return 0;
...
int WiFiUDP::beginMulticastPacket(){
  if(!server_port || multicast_ip == INADDR_ANY)
    return 0;
...
int WiFiUDP::beginPacket(in_addr_t ip, uint16_t port){
  remote_ip = ip;
  remote_port = port;
  return beginPacket();
}
...
int WiFiUDP::beginPacket(const char *host, uint16_t port){
  ...
  if (server == NULL){
    ESP_LOGE(TAG,"could not get host from dns: %d", errno);
    return 0;
  }
  return beginPacket((server->h_addr_list[0]), port);
}
