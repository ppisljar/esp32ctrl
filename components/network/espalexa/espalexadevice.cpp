...
EspalexaDevice::EspalexaDevice(std::string deviceName, std::function<void(void*,uint8_t)> gnCallback, EspalexaDeviceType t, uint8_t initialValue) { //constructor for general device
...
std::string EspalexaDevice::getName()
{
  return _deviceName;
}
...
uint32_t EspalexaDevice::getRGB()
{
  ...
  if (_mode == EspalexaColorMode::ct)
  {
    ...
    rgb[0] = (unsigned char)constrain(r,0.1,255.1);
    rgb[1] = (unsigned char)constrain(g,0.1,255.1);
    rgb[2] = (unsigned char)constrain(b,0.1,255.1);
  } else if (_mode == EspalexaColorMode::hs)
  {
    float h = ((float)_hue)/65535.0;
    float s = ((float)_sat)/255.0;
    unsigned char i = floor(h*6);
    float f = h * 6-i;
    float p = 255 * (1-s);
    float q = 255 * (1-f*s);
    float t = 255 * (1-(1-f)*s);
...
void EspalexaDevice::setName(std::string name)
{
  _deviceName = name;
}
...
{
  if (_callback != nullptr) {_callback(this, _val); return;}
}